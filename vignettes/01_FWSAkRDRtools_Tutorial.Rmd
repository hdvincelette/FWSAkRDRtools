...
You can restrict the search to the top level of a specified subfolder path (`recursive = FALSE`).
Otherwise, the function will search subsequent subfolders (`recursive = TRUE`).
Both the main (`main = TRUE`) and "incoming" (`incoming = TRUE`) folders will be searched unless you exclude one or the other by changing the argument to `FALSE`.
You can also specify whether results include the full file path (`full.path = TRUE`) or just the subfolder path(s) and file name(s) (`full.path = FALSE`).
